...
Wiki中文公开资料链接：https://dumps.wikimedia.org/zhwiki/latest/。

| 文件后缀             | 描述                                                         |
| -------------------- | ------------------------------------------------------------ |
| .xml.gz              | 压缩的XML文件，存储结构化数据，如维基百科文章和元数据        |
| .xml.gz-rss.xml      | 包含关于`.xml.gz`文件的更新和元信息的RSS文件                 |
| .sql.gz              | 压缩的SQL文件，包含可以导入到数据库的SQL命令，用于数据的备份和迁移 |
| .sql.gz-rss.xml      | 包含关于`.sql.gz`文件的更新和元信息的RSS文件                 |
| .bz2                 | 适用于大数据量的存储，经过Bzip2压缩之后的文件，较高的压缩率  |
| .multistream.xml.bz2 | 包含了多个独立数据流的压缩文件，允许快速访问文件中的特定部分 |
| .jsonl               | 包含每行为一个独立的JSON对象的数据文件，便于流式处理和逐行解析 |

每种文件类型的特点和用途

| 文件名后缀                 | 描述                                                         |
| -------------------------- | ------------------------------------------------------------ |
| pages-articles             | 完整的文章内容。                                             |
| abstract                   | 文章的摘要信息。                                             |
| pages-articles-multistream | 完整的文章信息，多数据流格式存储，允许快速访问压缩文件的特殊部分 |
| pages-meta-current         | 包含最新版本的文章页面元数据                                 |
| stub-articles              | 包含内容比较短，小作品文章的文件。                           |
| stub-meta-history          | 包含小作品文章和编辑历史信息。研究文章发展过程和编辑模式     |
...
常用的事`zh_core_web_lg`处理中文文本，托管在spacy平台。

模型的主要作用：

1. 分词：将文本切割成单独的词汇或者标记。
2. 词性标注：标记每一个词汇的词性
3. 命名实体识别：识别文本中的特定实体
4. 依存句法分析：分析词汇之间的依存关系，帮助理解句子结构

[Spacy官方](https://spacy.io/)、

[zh_core_web_lg](https://spacy.io/models/zh#zh_core_web_lg)

```shell
python -m spacy download zh_core_web_lg
```

![image-20251116204922631](images/百晓知识库/image-20251116204922631.png)

上述是WIKI数据处理 代码逻辑脚本流程。

接下来：执行数据处理脚本：将wiki数据转成jsonl格式。
...
python preprocess_wiki.py 
  # 存放wiki 数据的位置
	--dump_path /root/****/***.bz2 
  # 处理完之后存放的路径
  --save_path /root/****/**.jsonl
```

```python
python preprocess_wiki.py --dump_path /Users/a1234/Downloads/hw-chat-0.2/knowledge_base/wiki/wiki_data/zhwiki-latest-stub-articles.xml.gz --save_path /Users/a1234/Downloads/hw-chat-0.2/knowledge_base/wiki/education.jsonl
...
**解析PDF最大的困难在于准确提取整个页面的内容并将内容（表格、标题、段落和文档）转成文档的文本形式。该过程涉及到处理文本的提取、图像识别的不准确以及表格中的行列关系处理混乱。**

主流方法：

- 基于规则的方法：每一个文档的风格和内容根据文档的组织特征确定，然而，这种方式的通用性不强，因为PDF类型和布局多样，不可能使用预定义的规则覆盖所有的类型和布局。
- 基于深度学习模型的方法：结合目标检测（yolox）和OCR方法。
- 基于多模态大模型传递复杂的结构或提取PDF中的关键信息。

##### 基于规则方法

最具代表意义的是pypdf，他是一种广泛使用的基于规则的解析器，也是Langchain和LlamaIndex解析PDF的标准方法，
...
解析结果：![image-20251117001845131](images/百晓知识库/image-20251117001845131.png)
...
优点是能够准确识别整个文档的布局，包括表格和段落。甚至可以理解表格中的内容。这意味着他可以将文档划分准确，划分出完整的信息单元，保留预期的含义和结构。
...
|        | 方案    | 特点                                                         |
| ------ | ------- | ------------------------------------------------------------ |
| 方案一 | Trulens | 1. 上手较为简单<br />2. 与LlamaIndex结合较好<br />3. 评估指标较少（3个）<br />4. 测试问题需要自己准备 |
| 方案二 | Ragas   | 1. 监控指标较多<br />2. Langchain结合较好<br />3. 端到端评估流程 |
| 方案三 | 自研    | 复杂                                                         |
| 方案四 | 人工    | 效果好                                                       |

#### 核心指标

![image-20251123212808338](images/百晓知识库/image-20251123212808338.png)

- Question： 用户的查询。某一些工具中，也叫做Input或者 Query。
- Context： 检索到的文档上下文。有些工具称之为：Retrival Context。
- Answer： 生成的答案。可以使用不同的术语，Actual Output 或者 Response。
- Ground Truth： 手动标记的正确答案。用于评估准确性。

#### Trulens

优先考虑了三个主要相关性：答案相关性、上下文相关性和依据性。
...
![image-20251123214725251](images/百晓知识库/image-20251123214725251.png)
