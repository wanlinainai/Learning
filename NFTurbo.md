...
> 设计亮点：
>
> 我们为了在开发的时候快速的进行测试，同时不用支付高额的链账户金币，采用的是Mock。通过工厂模式支持链和Mock场景。
>
> ```java
> @Service
...
其中可能包含了支付的幂等校验（由于支付的时候可以选择不同的渠道支付，微信支付之后支付宝也可以支付成功）。

我们主要来看支付成功部分。

整个支付功能涉及到4个模块：支付模块、订单模块、库存模块、链模块。

我们使用的就是seata来保证事务的最终一致性。选用的是AT 模式。

![image-20260204232332799](images/NFTurbo/image-20260204232332799.png)

接入 AT 模式的代码还是非常简单的。将原本本地事务实现的注解`@Transational`注解换成`@GlobalTransational`即可。Seata会通过我们搭建好的服务端进行交互，进行整个分布式事务的协调。

Seata中包含了三个组件：

1. **TC**：事务协调器。管理全局事务的生命周期，包含开始事务、提交事务和回滚事务。Seata服务端
2. **TM**：事务管理器。定义事务的范围，负责开启和结束全局事务。
3. **RM**：资源管理器。管理资源对象，如数据库连接，负责资源的注册和释放

在我们的系统中，TM是我们的支付模块，TC是Seata服务端，RM就是订单模块、库存模块。

使用了Seata之后，一次分布式事务的大致流程如下：

1. TM（支付模块）接收到支付成功的回调请求之后，去TC（Seata服务端）开启一个新的全局事务，并且从TC获取到他的XID（全局事务ID）。
2. TM（支付模块）开始通过Dubbo调用各个RM（订单或者库存），调用的时候将XID传递过去。
3. RM（订单、藏品等服务）通过接收到XID，将各自管理的资源且该调用使用到的资源注册成一个事务分支（Branch Transation）。
4. 当该请求全局结束之后，TM（支付模块）根据本地调用的结果是否存在失败的情况进行决议处理，都成功，决议COMMIIT，出现失败，决议ROLLBACK。
5. TM（支付模块）将决议的结果通知到TC（Seata服务端），TC协调各个RM进行事务的二阶段动作，该回滚的回滚，提交的提交。

> 回滚的实现是通过**undo_log**实现的，如果我们存在多个模块，用的是不同的 数据库，都需要单独在数据库中创建undo_log表，这个undo_log和innodb的undo_log不一样。需要根据Seata官方推荐的部署方式部署。

我们在**paySuccess**中加上注解：`@GlobalTransactional`，不论是在远程调用中还是这个方法中触发了异常抛出异常，都会被拦截，之后做回滚处理。

> 需要注意：每一个服务需要单独启动，如果存在一个模块包裹了其他的一些模块，需要不同的模块单独启动才可以。
...
用户在某一些稀有藏品出售之前需要先预约，之后才能购买。

其中有一张预约表：`goods_book`

![image-20260304171500151](NFTurbo/image-20260304171500151.png)

主要就是一些商品信息、买家信息和预约时间信息相关，还有什么乐观锁版本号和幂等号等。

其中和`collection`藏品表相关联的字段属性有：`can_book`、`book_start_time`、`book_end_time`。

流程：

![image-20260304172707526](NFTurbo/image-20260304172707526.png)
...
### TCC实现普通交易环节订单和库存的扣减一致性

作为普通交易部分，主要处理的是非秒杀流量，那么就意味着没有热点数据和热点行的更新，下单的逻辑其实就是库存的扣减、创建订单，之后异步将库存同步到Redis中即可。

相较于秒杀方案，秒杀的话不管是哪一种，其实都是通过在同步链路上进行库存的预扣减，之后通过异步链路进行数据库库存的扣减和订单的创建，这个是通过MQ的事务消息保证了事务的最终一致性。但是普通交易并不存在这个重试机制，那么我们就引入 TCC 来保障最终一致性。

#### TCC介绍

TCC 是一种基于业务补偿的分布式事务解决方案，核心思想就是通过三个阶段保证分布式事务的一致性。（**Try、Confirm、Cancel**）。

1. Try（尝试）：各项参与服务预业务检查（资源预留）。比如说 库存的冻结，保证一个资源只能本人进行操作，其他人不能操作，这样的话也同时保证了第二步的执行成功率。
2. Confirm（确认）：Try执行成功之后，协调者触发Confirm 操作，各个服务正式提交事务（实际的库存扣减、划转资金等）。
3. Cancel（取消）：任何一个Try失败之后都会触发Cancel操作，各个服务回滚到冻结时候的预留资源状态（解冻库存、返还预扣的自金额）。

一般来说我们的 Confirm 确认的时候出现问题也会进行 Cancel 操作，但是并不是直接进行的，其实是先进行自身的重试，重试很多次之后如果还是失败在执行 Cancel ，回滚掉之前的库存等预留资源信息。

> TCC 扩展：
>
> **TCC不是一定保证成功，**假设Confirm一直失败重试，可能的原因是数据库挂掉了或者是网络问题，TCC会通过事务表的方式将具体的操作事务存储到数据库中，通过状态机和后台补偿线程进行重试，直到网络恢复或者数据库重启成功就会执行成功。如果是极端情况：数据被误删除了或者代码出错了，这种的话可以采用人工干预的方式进行后续的处理，该回滚的回滚，该取消的取消。

#### 引入事务表解决TCC空回滚和悬挂问题

TCC 中存在着空回滚和悬挂的问题。

1. **空回滚问题**：TCC的Try尝试的过程中，有的参与者成功了，但是有的参与者失败了，这个时候需要所有参与者都执行Cancel，这个时候对于那一些已经成功的可以执行Cancel，对于没有Try成功的来说就是一个空回滚了。需要在业务中做好空回滚的处理，否则会抛出错误异常，甚至可能导致Cancel一直失败，最终导致整个分布式事务全部失败。
2. **悬挂事务问题**：在调用一阶段的Try操作的时候可能会由于网络阻塞导致超时，此时事务协调器会触发二阶段回滚，调用TCC服务Cancel操作；但是此时如果网络阻塞结束了，拿到这个Try的数据包了，就会出现Cancel比Try优先执行的问题。其实这种就是一种事务悬挂的问题。

是如何处理的呢？我们引入了一张表：**transaction_log**表。
...
- transaction_id：事务ID，只需要保证唯一ID，可以采用订单ID。
- business_scene：业务场景。
- business_module：业务模块，比如订单模块、藏品模块、盲盒模块等。
- state： 其实就是三个状态：Try、Confirm、Cancel。
- cancel_type： 取消类型。如果此次是Cancel状态的话，他的取消类型是什么。
  - CANCEL_AFTER_TRY_SUCCESS
  - CANCEL_AFTER_CONFIRM_SUCCESS
  - EMPTY_CANCEL：空回滚
  - DUPLICATED_CANCEL：幂等成功

`transaction_log`这张表需要和业务的表在同一个数据库的，需要借助数据库本地事务保证ACID。

##### 三种情况下表中的信息

**如果是Try-Cancel的事务（库存Try成功，订单Try失败，执行Cancel操作）。**

Order模块中的`transaction_log`表结果：

![image-20260301205653015](images/NFTurbo/image-20260301205653015.png)

藏品模块中的`transaction_log`表结果：

![image-20260301205732202](images/NFTurbo/image-20260301205732202.png)

> 分析：Order订单模块没有Try执行成功，所以压根不存在从Try到Cancel的过程，直接执行Cancel了，取消类型也就是：`EMPTY_CANCEL（空回滚）`。
>
> 藏品模块中执行成功了Try，但是在确认Confirm的时候报错了，说明我们需要取消的是Try状态，也就是：`CANCEL_AFTER_TRY_SUCCESS`状态。

**如果Try-Comfirm的事务。**

Order模块中的`transaction_log`表结果：

![image-20260301210321139](images/NFTurbo/image-20260301210321139.png)

盲盒模块中的`transaction_log`表结果：

![image-20260301210350747](images/NFTurbo/image-20260301210350747.png)

> 由于没有相关的错误取消操作，所以cancel_type是null

**如果是Try-Confirm-Cancel的事务流程（库存Try成功，Confirm成功，Cancel成功。订单Try成功，Confirm失败，Cancel成功）。**`transaction_log`表的结果如下：

Order模块：

![image-20260301210549177](images/NFTurbo/image-20260301210549177.png)

BlindBox模块：

![image-20260301210622605](images/NFTurbo/image-20260301210622605.png)

> 在订单模块中，由于Try执行成功了，但是Confirm执行失败了，所以取消类型应该是：`CANCEL_AFTER_TRY_SUCCESS`，BlindBox模块中，由于Try和Confirm都执行成功了，所以最终的Cancel_type是：`CANCEL_AFTER_CONFIRM_SUCCESS`。

##### 代码实现

我们的TCC相关轮子代码都是在`nft-turbo-tcc`模块下，我们将这个内容封装成了Starter，其中核心的代码是：`transactionLogService`。这个Service我们没有添加`@Service`注解，是使用Spring 3 的自动注入的方式（`org.springframework.boot.autoconfigure.AutoConfiguration.imports`）将Configuration导入，这个配置文件中进行Bean的初始化：

**TccConfiguration.java**
...
核心部分是：`TransactionLogService.java`类。其中有三个方法：`tryTransaction`、`confirmTransaction`、`cancelTransaction`。

- tryTransaction方法中主要是表中的取消状态设置成TRY。当然如果出现重复的话需要幂等掉，就是设置成`DUPLICATED_TRY`状态了。
...
2. 针对于数据库连接部分的处理：主要原因其实是数据库配置的链接数量太少了，以及我们之前的关于druidDataSource的配置有点问题，修改为：
   ...
   datasource:
     ...
     max-active: 100 # 连接池中允许的最大连接数。如果所有连接都被使用并且没有空闲连接，新的连接请求将被阻塞，直到有连接可用。默认值为8。
...
如此一来我们感觉瓶颈应该不是在线程数上了，我们着手数据库的连接数。将连接数从原本的50调整到100，结果时间为16S。但是数据库的IOPS（IO操作）很高，很可能是出现在这个问题上。

可能问题都不在这些地方。

突然发现SQL存在慢日志，我们决定着手调查这个慢日志出现的原因。（锁等待）即使我们加了MQ做削峰填谷，但是我们的数据库还是存在慢日志，如果真的是这样的话，我们就需要考虑加限流了。

我们设置的线程数比较小的时候没有慢SQL，但是存在MQ的积压；如果设置的线程数大的话就会存在慢SQL，MQ不会积压。

**接下来的步骤就是针对线程数进行调整，为啥还要做线程池参数的调整呢？因为我们的线程数大的时候会出现慢SQL，但是小的时候会出现MQ消息堆积。我们只能先从动态调整线程数的方式来尝试能否找到一个消费快同时没有慢SQL的状态。**

- 8个核心线程，16个最大线程 ----->  26S, SQL < 100ms
- 16个核心线程，32个最大线程  ----->  12S， SQL< 100ms
- 32个核心线程， 64个最大线程  ----->  11S，没有慢SQL，SQL == 110ms
- 64个核心线程，64个最大线程  ----->  12S， 没有慢SQL， SQL == 280ms
- 24    核心线程，64个最大线程  ----->  11S， 大概是在90ms左右
- 20个核心线程，64个最大线程  ----->  11S，SQL < 100ms
- 18个核心线程，64个最大线程  ----->  12S，SQL < 100ms

上述这种的话其实是不能通过硬件设备的提升解决的，因为其实线程数量多了之后其实update的时候存在锁的操作，线程数越高锁的线程数量越多。

目前我们的MQ + Redis的方案就是11S的瓶颈了。

现在最好的方式就是加一个限流 200左右，因为我们的QPS是300嘛。

![image-20260311223437993](images/NFTurbo/image-20260311223437993.png)

### 针对上述的300QPS出现的瓶颈做优化

上述的压测（MQ + Redis）其实都是基于**AP**的思想进行处理的，保证的是最终的一致性，而不是高可用性。

> CAP理论：
>
> C：一致性。所有节点的数据在同一个时刻看到的数据是相同的。
>
> A：可用性。每一个服务一直可用，并且是正常响应时间。
>
> P：分区容错性。尽管网络节点中会出现丢弃任意数量的消息，系统依旧能够正常运行。

我们可以将订单的创建移动到MQ之前，通过引入限流机制控制速率。将库存和订单的创建拆分开。

![image-20260311225954919](images/NFTurbo/image-20260311225954919.png)

这种思想的话其实就是**AP**的思想，保证用户优先看到订单，之后用户可以下一步支付操作。通过Redis来保证库存不会超卖。

还有一个就是其实订单创建的数据库瓶颈并不会出现库存修改那么大，因为我们的订单ID是通过雪花算法生成的，不是自增主键一类的；但是库存扣减其实是一个比较耗时的操作，原因是update针对的是同一个热点行进行操作。insert是在B+树右侧节点追加。
...
对于一个电商系统来说，他们的QPS的关系是：

**商品详情页面/搜索QPS > 下单QPS > 支付QPS。**

按照我们的系统之前的压测，下单接口峰值 500 QPS来算，大致的<span style="color: #f8a212">预估</span>的流量模型如下：
...
这只是系统理论上的最大的QPS，实际上很可能更小。

##### 网关限流配置

基本上需要考虑到QPS的实际情况进行Sentinel的限流配置。

- goods模块：实际上的大流量应该都是集中在这个模块的，大概可能是5000 - 10000左右，因为都是读请求，其实不太会存在瓶颈问题。
- trade模块：主要承接的是创单、支付等接口QPS，需要抗下下单和支付单之类的所有流量的话就计算总和，基本上要求3000QPS，设置3000的限流即可。我们是往大了预估的。
- Order模块：主要抗住的是创建订单，500QPS，限流设置700QPS。
- inventory模块：最高的压测其实只有500QPS，所以我们设置500即可。
- Pay模块：同inventory模块。
- 其他模块不设置限流。
...
旁路验证其实就是几乎实时逐条核对。我们的业务中在执行下单的时候，主线程其实是扣减Redis的库存，之后消费者监听消息做数据库的库存扣减。

我们完全可以在Redis扣减成功之后在主线路中执行旁路验证，通过延迟3S的时间进行与数据库的对比库存的扣减数量是否一致即可。

![image-20260314000247118](images/NFTurbo/image-20260314000247118.png)
