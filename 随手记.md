...
fail-Fast本质上是一种快速失败的思想，比如我们在平时写业务逻辑代码的时候前置的一般都是一些校验判断，比如用户校验，幂等号校验等等都是放在逻辑的最前方，目的就是为了快速失败。之后的逻辑不在执行了，达到快速失败。

除了上述的思想，还有一种是在集合中的错误检测机制，多个线程对同一个集合进行操作的时候和可能会出现报错异常：
...
![image-20260314152948524](随手记/image-20260314152948524.png)

原因其实是其中有一个数量比较的操作：位置在：`ArrayList$Itr.checkForComodification`
...
modCount 指的是集合修改的次数，初始化的时候就是1，expectedModCount指的是预期被修改的次数

fail-safe是为了处理上述的问题的，本质操作是在添加删除的时候拷贝原有的集合，在新的集合上操作。CopyOnWriteArrayList。
...
1. 考虑查询的效率和频率，不经常查询的不给建设索引
2. 适当的索引类型：B+Tree索引、Hash索引、全文索引等
3. 考虑区分度：尽量不要选择区分度不高的字段作为索引，比如性别。
4. 优先考虑联合索引
5. 考虑到索引覆盖
